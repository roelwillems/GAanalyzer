// ==UserScript==
// @name GA Analyzer GA.js
// @namespace http://andrescholten.nl/index.php/greasemonkey-script-voor-google-analytics/
// @description GA Analyzer shows if GA is implemented and if there are GA cookies
// @include *
// @exclude *.google.com/*
// @exclude www.google.*/*
// ==/UserScript==
...
for (x in document.getElementsByTagName("script"))
{
  ...
  if (i.innerHTML)
  { 
    uacode = /(\s)*_gat/._getTracker(\s)*=(\s)*\"(.*)\"/gi.exec(i.innerHTML);
...
{ 
  ...
  if (calls_on_site == 0 ) { msg("<span style='color: #FF0000;'>No GA.jsTracker() calls found</span>"); }
  if (calls_on_site == 1 ) { msg("<span style='color: #FFFF00;'>" + calls_on_site + "</span> GA.jsTracker() call found"); }
  else { msg("<span style='color: #FFFF00;'>" + calls_on_site + "</span> GA.jsTracker() calls found"); }
}
